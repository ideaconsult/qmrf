...
public class QMRFObject extends AmbitObject implements InterfaceQMRF, IAmbitObjectListener {
	...
	protected String[][] chaptersID = { { "QSAR_identifier", "QSAR identifier" },
			...
			{ "QSAR_Miscelaneous", "Miscellaneous information" }, { "QMRF_Summary", "Summary (JRC Inventory)" } };
	...
	public QMRFObject(String[] args, boolean adminUser) {
		 ...
		 * "org.apache.xerces.parsers.SAXParser"); System.setProperty(
	...
	public String getNumber() {
		try {
			QMRFSubChapterText ch10 = (QMRFSubChapterText) chapters.get(chapters.size() - 1).getSubchapters()
					.getItem(0);
			return ch10.getText();
		} catch (Exception x) {
			return "";
	...
	/**
	 * Inserts strings into help attribute of QMRF chapters. Done by XSLT
	 * transformation. The stylesheets are
	 * ambit2/qmrfeditor/qmrf_insert_help.xsl and
	 * ambit/data/qmrf/qmrf_delete_help.xsl.
	 * 
	 * @param doc
	 * @param appendHelp
	 * @return
	 ...
	 * 
	 * @param xslt
	 *            XSLT stylesheet (e.g. XSL file generating HTML from QMRF)
	 * @param out
	 *            The result (e.g. HTML presentation of QMRF document).
	 * @throws IOException
	 ...
	 * @throws TransformerException
	 */
	protected void transform(Source sourceDocument, Source xslt, Result result)
	...
	/*
	 * public void export2PDF(InputStream xsltfo, OutputStream pdf) throws
	 * Exception { Document doc = buildDocument(); DocumentBuilderFactory
	 * factory = DocumentBuilderFactory.newInstance(); DocumentBuilder builder =
	 * factory.newDocumentBuilder(); Document xslt = builder.parse(xsltfo);
	 * DOMResult result = new DOMResult(); transform(new DOMSource(doc),new
	 * DOMSource(xslt), result); Document foDom = (Document) result.getNode();
	 ...
	 * 
	 * @param foDom
	 *            Document FO namespace
	 *            xmlns:fo="http://www.w3.org/1999/XSL/Format"
	 * @param pdf
	 * 
	 *            protected void convertDOM2PDF(Document foDom, OutputStream
	 *            pdf) { try {
	 * 
	 * 
	 *            // configure fopFactory as desired FopFactory fopFactory =
	 *            FopFactory.newInstance();
	 * 
	 *            // xsl-fo namespace URI String foNS =
	 *            "http://www.w3.org/1999/XSL/Format"; FOUserAgent foUserAgent =
	 *            fopFactory.newFOUserAgent(); // configure foUserAgent as
	 *            desired
	 * 
	 *            // Setup output
	 * 
	 *            BufferedOutputStream out = new
	 *            java.io.BufferedOutputStream(pdf);
	 * 
	 *            try { // Construct fop with desired output format and output
	 *            stream Fop fop = fopFactory.newFop(MimeConstants.MIME_PDF,
	 *            foUserAgent, out);
	 * 
	 *            // Setup Identity Transformer TransformerFactory xfactory =
	 *            TransformerFactory.newInstance(); Transformer transformer =
	 *            xfactory.newTransformer(); // identity transformer
	 * 
	 *            // Setup input for XSLT transformation Source src = new
	 *            DOMSource(foDom);
	 * 
	...
	protected Options createOptions() {
		...
		Option dtdschema = OptionBuilder.withLongOpt("dtd").withArgName("url").hasArg()
				.withDescription(
						"DTD schema location - URL where DTD schema resides (e.g. -dfile:///D:/myfolder/qmrf.dtd or -dhttp://qmrf.sourceforge.net/qmrf.dtd ) ")
				.create("d");

		Option content = OptionBuilder.withLongOpt("xmlcontent").withArgName("URL").hasArg()
				.withDescription("URL to retrieve XML content").create("x");

		Option ttf = OptionBuilder.withLongOpt("ttf").withArgName("URL").hasArg()
				.withDescription("URL to retrieve TrueType font").create("t");

		Option help = OptionBuilder.withLongOpt("help").withDescription("This screen").create("h");

		Option admin = OptionBuilder.withLongOpt("user").withArgName("username").hasArg()
				.withDescription("User (if -u admin then Chapter 10 will be editable, otherwise readonly").create("u");

		Option endpoints = OptionBuilder.withLongOpt("external").withArgName("URL").hasArg()
				.withDescription(
		...
		Option catalog_cleanup = OptionBuilder.withLongOpt("cleancatalogs")
				.withDescription(
						"true| false : When saving as XML, include only catalog entries which have an idref reference")
				.withArgName("value").create("c");
	...
	protected void setOptions(CommandLine line) {
		...
		if (external_catalogs.size() == 0) {
			...
			try {
				...
				props.load(in);
				String pa = props.getProperty("authors");

				readExternalCatalogs(pa);
			} catch (Exception x) {
	...
	public void cleanCatalogs() {
		...
		/*
		 * cc = c.values().iterator(); while (cc.hasNext()) { Catalog a =
		 * cc.next(); for (int k=a.size()-1; k >= 0;k--) if
		 * (!a.getItem(k).isSelected()) { a.remove(k); } }
		 */
	}
...
class SimpleDeclHandler implements org.xml.sax.ext.DeclHandler {
	...
	public void attributeDecl(java.lang.String elementName, java.lang.String attributeName, java.lang.String type,
